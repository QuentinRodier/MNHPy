...
class PanelPlot():
    ...
    def __init__(self, nb_l, nb_c, Lfigsize, bigtitle, titlepad=40, minmaxpad=1.03, timepad=-0.06, lateralminmaxpad=0.86,
        ...
        # Colorbar pad (horizontal shift from graph)
        self.colorbarpad = colorbarpad
        self.lateralminmaxpad = lateralminmaxpad
        self.labelcolorbarpad = labelcolorbarpad  # Vertical colorbal label pad
        # Ratio of long to short dimensions of colorbar w.r.t. the figure
        self.colorbaraspect = colorbaraspect
    ...
    def draw_Backmap(self, drawCoastLines, ax, projo):
        ...
        if 'PlateCarree' in str(projo):
            gl = ax.gridlines(crs=self.projo, draw_labels=True,
    ...
    def set_Title(self, ax, i, title, Lid_overlap, xlab, ylab):
        ...
        if not Lid_overlap:
            self.ax[self.i].set_title(
                title, pad=self.titlepad, fontsize=self.titleSize)
        else:  # If graph overlap, title is concatenated
            new_title = self.ax[self.i].get_title() + ' and ' + title
            self.ax[self.i].set_title(
                new_title, pad=self.titlepad, fontsize=self.titleSize)
    ...
    def set_XYaxislab(self, ax, i, xlab, ylab):
        ...
        if 'GeoAxes' in str(type(self.ax[self.i])):
            ...
            self.ax[self.i].set_xlabel(
                xlab, fontsize=self.xlabelSize, labelpad=0.1)
            self.ax[self.i].set_ylabel(
                ylab, fontsize=self.ylabelSize, labelpad=0.1)
    ...
    def addLine(self, ax, beg_coord, end_coord, color='black', linewidth=0.2):
        ...
        x1, y1 = [self.beg_coord[0], self.end_coord[0]], [
            self.beg_coord[1], self.end_coord[1]]
        ax.plot(x1, y1, color=self.color, linewidth=self.linewidth)

    def print_minmax(self, var, title):
        print(str(title) + "   min = " + str(np.nanmin(var)) +
              "  max = " + str(np.nanmax(var)))
    ...
    def set_minmaxText(self, ax, i, var, title, Lid_overlap, facconv):
        ...
        strtext = "   min = " + \
            "{:.3e}".format(np.nanmin(var*facconv)) + "  max = " + \
            "{:.3e}".format(np.nanmax(var*facconv))
        if not Lid_overlap:
...
#      D = initialize_default_val(Lvar, D)
        ...
        if not Lcolormap and not LcolorLine:
            # If no color given, a cmap is given
            Lcolormap = ['gist_rainbow_r']*len(Lvar)
        if not Lcolormap:
        ...
        # The extra value is 1E-6 times the step ticks of the colorbar
        Lmaxval = list(map(lambda x, y: x + 1E-6*y, Lmaxval, Lstep))
        ...
        for i, var in enumerate(Lvar):
            if firstCall:  # 1st call
                ...
                iax = Lid_overlap[i]
            # existing ax with no overlapping (graphd appended to existing panel)
            else:
                self.ax.append(self.fig.add_subplot(
                    self.nb_l, self.nb_c, self.nb_graph+1))
                self.nb_graph += 1
                # The ax index of the new coming plot is the length of the existant ax -1 for indices matter
                iax = len(self.ax)-1
            ...
            self.set_minmaxText(self.ax, iax, var,
                                Ltitle[i], Lid_overlap, Lfacconv[i])
            ...
            if LaddWhite_cm[i] and Lcolormap:
                Lcolormap[i] = self.addWhitecm(
                    Lcolormap[i], len(levels_contour), LwhiteTop[i])
            ...
            #  Title
            self.set_Title(
                self.ax, iax, Ltitle[i], Lid_overlap, Lxlab[i], Lylab[i])

            #  X/Y Axis label
            self.set_XYaxislab(self.ax, iax, Lxlab[i], Lylab[i])

            #  Ticks label
            self.ax[iax].tick_params(axis='both', labelsize=self.xyTicksLabelSize,
                                     width=self.xyTicksWidth, length=self.xyTicksLength, pad=self.tickspad)
            ...
            #  Colorbar
            if colorbar:
                cb = plt.colorbar(cf, ax=self.ax[iax], fraction=0.031, pad=self.colorbarpad, ticks=np.arange(
                    Lminval[i], Lmaxval[i], Lstepticks[i]), aspect=self.colorbaraspect)
                # This creates a new AxesSubplot only for the colorbar y=0 ==> location at the bottom
                cb.ax.set_title(
                    Lcbarlabel[i], pad=self.labelcolorbarpad, loc='left', fontsize=self.cbTitleSize)
    ...
    def pXY_lines(self, Lxx=[], Lyy=[], Lxlab=[], Lylab=[], Ltitle=[], Llinetype=[], Llinewidth=[],
        ...
        if firstCall:  # 1st call
            iax = 0
            self.ax.append(self.fig.add_subplot(
                self.nb_l, self.nb_c, 1, label='graph axe x down'))
            self.nb_graph += 1
        elif id_overlap:  # overlapping plot with a different x-axis
            self.ax.append(self.fig.add_subplot(
                self.nb_l, self.nb_c, self.nb_graph, label='graph axe x top', frame_on=False))
            iax = len(self.ax)-1
        # existing ax with no overlapping (graph appended to existing panel)
        else:
            self.ax.append(self.fig.add_subplot(
                self.nb_l, self.nb_c, self.nb_graph+1))
            self.nb_graph += 1
        ...
        for i, var in enumerate(Lxx):
            ...
            if not id_overlap:
                self.ax[iax].legend(loc='upper right', bbox_to_anchor=(
            ...
            if Ltitle:
                self.set_Title(
                    self.ax, iax, Ltitle[i], id_overlap, Lxlab[i], Lylab[i])
...
#      D = initialize_default_val(Lvar, D)
        ...
        for i, var in enumerate(Lvar):
            if firstCall:  # 1st call
                ...
                if Lproj:
                    self.ax.append(self.fig.add_subplot(
                        self.nb_l, self.nb_c, i+1, projection=Lproj[i]))
                else:  # Cartesian coordinates
                    self.ax.append(self.fig.add_subplot(
                        self.nb_l, self.nb_c, i+1))
                self.nb_graph += 1
            elif Lid_overlap != []:  # overlapping plot
                iax = Lid_overlap[i]
            # existing ax with no overlapping (graphd appended to existing panel)
                ...
                if Lproj:
                    self.ax.append(self.fig.add_subplot(
                        self.nb_l, self.nb_c, self.nb_graph+1, projection=Lproj[i]))
                else:  # Cartesian coordinates
                    self.ax.append(self.fig.add_subplot(
                        self.nb_l, self.nb_c, self.nb_graph+1))
                self.nb_graph += 1
            ...
            #  Print min/max (stout and on plot)
            self.set_minmaxText(self.ax, iax, vartoPlot,
                                Ltitle[i], Lid_overlap, Lfacconv[i])
            ...
            #  Color label on contour-line
            if Lpltype[i] == 'c':  # Contour
                # cartopy does not like the levels arguments in clabel, known issue
                if 'GeoAxes' in str(type(self.ax[self.i])):
                    self.ax[iax].clabel(cf, fontsize=self.cbTicksLabelSize)
                else:
                    self.ax[iax].clabel(cf, levels=np.arange(
                        Lminval[i], Lmaxval[i], step=Lstep[i]), fontsize=self.cbTicksLabelSize)
            ...
            if colorbar:
                ...
                cb.ax.set_title(
                    Lcbarlabel[i], pad=self.labelcolorbarpad, loc='left', fontsize=self.cbTitleSize)
                if Lcbformatlabel[i]:
                    cb.ax.set_yticklabels(["{:.1E}".format(i)
    ...
    def pvector(self, Lxx=[], Lyy=[], Lvar1=[], Lvar2=[], Lcarte=[], Llevel=[], Lxlab=[], Lylab=[],
        ...
        for i, var1 in enumerate(Lvar1):
            ...
            self.set_minmaxText(self.ax, iax, np.sqrt(
                vartoPlot1**2 + vartoPlot2**2), Ltitle[i], Lid_overlap, Lfacconv[i])
            ...
            if Lxx[i].ndim == 2:
                cf = self.ax[iax].quiver(axeX[::Larrowstep[i], ::Larrowstep[i]], axeY[::Larrowstep[i], ::Larrowstep[i]],
                                         vartoPlot1[::Larrowstep[i], ::Larrowstep[i]
                                                    ], vartoPlot2[::Larrowstep[i], ::Larrowstep[i]],
            ...
            # Arrow legend key
            qk = self.ax[iax].quiverkey(cf, 1.0, -0.05, Llegendval[i], str(
                Llegendval[i]) + Llegendlabel[i], labelpos='E', color='black')
    ...
    def pstreamline(self, Lxx=[], Lyy=[], Lvar1=[], Lvar2=[], Lcarte=[], Llevel=[], Lxlab=[], Lylab=[], Llinewidth=[], Ldensity=[],
        ...
        for i, var1 in enumerate(Lvar1):
            ...
            #  Plot
            cf = self.ax[iax].streamplot(Lxx[i], Lyy[i], vartoPlot1, vartoPlot2,
                                         density=Ldensity[i], linewidth=Llinewidth[i], color=Lcolor[i])
    ...
    def pXY_bar(self, Lbins=[], Lvar=[], Lxlab=[], Lylab=[], Ltitle=[], Lcolor=[], Lwidth=[],
        ...
        for i, var in enumerate(Lvar):
            if firstCall:  # 1st call
                iax = i
                self.ax.append(self.fig.add_subplot(
                    self.nb_l, self.nb_c, i+1, label='graph axe x down'))
                self.nb_graph += 1
            elif id_overlap:  # overlapping plot with a different x-axis
                self.ax.append(self.fig.add_subplot(
                    self.nb_l, self.nb_c, self.nb_graph, label='graph axe x top', frame_on=False))
            ...
            #  Plot
            cf = self.ax[iax].bar(labels, var*Lfacconv[i], width=Lwidth[i],
                                  color=Lcolor[i], linewidth=Llinewidth[i], edgecolor=Llinecolor[i])

            #  Legend
            # TODO : Handling legend with overlap two axis lines in the same box. For now, placement is by hand
            if not id_overlap:
                self.ax[iax].legend(loc='upper right',
