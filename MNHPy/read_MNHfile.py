        ...
        for var in Dvar_input[keyFiles]:  # For each files
            ...
            if(theFile.format == 'FA'):
                ...
                if(var[1] == None or var[1] == 0):  # 2D Field
                    ...
                    Dvar_output[keyFiles][var[0] +
                                          str(var[1])] = theFile.readfield(var).getlevel(k=var[1])
            elif(theFile.format == 'netCDFMNH'):
                if(var[1] == None or var[1] == 0):  # 2D Field
                    Dvar_output[keyFiles][var[0]] = theFile.readfield(var[0])
                else:
                    Dvar_output[keyFiles][var[0] +
                                          str(var[1])] = theFile.readfield(var[0]).getlevel(k=var[1])
                ...
                raise epygramError(
                    "Unknown format file, please use FA, LFI, GRIB or MNH NetCDF")
        theFile.close()
    ...
    for i, keyFiles in enumerate(Dvar_input.keys()):
        ...
        if '000' in LnameFiles[i][-6:-3]:
            if theFile['MASDEV'][0] <= 54:
                ...
                Dvar[keyFiles] = read_TIMESfiles_55(
                    theFile, Dvar_input[keyFiles], Dvar[keyFiles], get_data_only, del_empty_dim, removeHALO)
        else:
            Dvar[keyFiles] = read_BACKUPfile(
                theFile, Dvar_input[keyFiles], Dvar[keyFiles], get_data_only, del_empty_dim, removeHALO)
        # theFile.close()
    return Dvar
...
def read_var(theFile, Dvar, var_name, get_data_only, del_empty_dim=True, removeHALO=True):
    ...
    try:
        ...
        raise KeyError(
            "Group and variable name not found in the file, check the group/variable name with ncdump -h YourMNHFile.000.nc. You asked for variable : " + var_name)
...
def read_from_group(theFile, Dvar, group_name, var_name, get_data_only, del_empty_dim=True, removeHALO=True):
    ...
    if not get_data_only:
        ...
        if var_dim == 0:
            Dvar[(group_name, var_name)
                 ] = theFile[group_name].variables[var_name][0].data
        if var_dim == 1:
            Dvar[(group_name, var_name)
                 ] = theFile[group_name].variables[var_name][:]
        elif var_dim == 2:
            Dvar[(group_name, var_name)
                 ] = theFile[group_name].variables[var_name][:, :]
        elif var_dim == 3:
            ...
            for i in range(8):
                try:
                    if var_dim_name[i] == 'level' or var_dim_name[i] == 'level_w' or \
                            ...
                            var_dim_name[i] == 'nj' or var_dim_name[i] == 'nj_u' or var_dim_name[i] == 'nj_v':
                        if var_dim != 0:
                            Dvar[(group_name, var_name)] = removetheHALO(
                                i+1, Dvar[(group_name, var_name)])
                except:
                    break
        if del_empty_dim:
            Ldimtosqueeze = []
            ...
            for i in range(8):
                try:
                    ...
                    break
            Ldimtosqueeze = tuple(Ldimtosqueeze)
            Dvar[(group_name, var_name)] = np.squeeze(
                Dvar[(group_name, var_name)], axis=Ldimtosqueeze)
...
def read_BACKUPfile(theFile, Dvar_input, Dvar, get_data_only, del_empty_dim=True, removeHALO=True):
    ...
    Dvar['date'] = nc.num2date(
        Dvar['time'], units=theFile.variables['time'].units, calendar=theFile.variables['time'].calendar)
    for var in Dvar_input:
        if type(var) == tuple:
            Dvar = read_from_group(
                theFile, Dvar, var[0], var[1], get_data_only, del_empty_dim, removeHALO)
        else:
            Dvar = read_var(theFile, Dvar, var, get_data_only,
                            del_empty_dim, removeHALO)
